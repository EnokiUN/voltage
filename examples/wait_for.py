...
async def on_message(message): # The name for this function doesnt matter, but we'll be naming it on_message for this example.
  if message.content.lower() == "-send":
    ...
    msg = await bot.wait_for("message", check=lambda message: message.author.id != bot.user.id, timeout=30) # Assign this to a variable as it returns a Messageable object for later.
    await message.reply(f"I appreciate the message, {message.author.name}, ill remember your words.. `{msg.content}`") # Replying with the message content sent by the user.
